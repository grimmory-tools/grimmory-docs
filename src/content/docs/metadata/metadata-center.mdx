...
:::tip[Display Mode]
The Metadata Center can open as a full page or a popup dialog. Configure this in [View Preferences](/view-preferences) under Metadata Center Display Mode.
:::
...
Recommendations are computed by a similarity algorithm and cached for performance. They update when the [Update Book Recommendations](/tools/task-manager) task runs.

:::note[Enable Recommendations]
Similar Book Recommendations must be enabled in [Global Preferences](/tools/global-preferences) for this tab to show results.
:::
...
- See [Metadata Settings](/metadata/metadata-settings) for configuring which provider fields are visible in the Edit Metadata form.
- See [Fetch Configuration](/metadata/metadata-fetch-configuration) for controlling how automated metadata lookups behave.
