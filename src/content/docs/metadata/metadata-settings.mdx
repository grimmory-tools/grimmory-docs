...
slug: docs/metadata/metadata-settings
sidebar:
...
<DocsImage src={"/img/metadata/metadata-persistence"} alt={"Metadata Persistence"} />
...
When enabled, Grimmory automatically moves and renames files according to their library's [file naming pattern](/docs/metadata/file-naming-patterns) whenever metadata is updated. For example, if your naming pattern includes the author and title, renaming a book or correcting an author will cause the file to be relocated to match the new metadata.
...
<DocsImage src={"/img/metadata/sidecar-json"} alt={"Sidecar JSON Files"} />
...
You can also bulk export and import sidecar files per library from the [Fetch Configuration](/docs/metadata/metadata-fetch-configuration#sidecar-export--import) page.
:::
...
<DocsImage src={"/img/metadata/auto-metadata-download"} alt={"Automatic Metadata Download"} />
When enabled, Grimmory will automatically retrieve metadata from your configured sources whenever new files are added to the Bookdrop folder. The metadata fetch uses the provider priority configuration set in the [Fetch Configuration](/docs/metadata/metadata-fetch-configuration) page.
...
<DocsImage src={"/img/metadata/metadata-providers"} alt={"Metadata Providers"} />
...
|----------|--------------|---------|
| **Amazon** | Region, Cookie | Select your Amazon region from 19 available domains (amazon.com, amazon.de, amazon.co.uk, etc.). Providing your Amazon session cookie is optional but **highly recommended** as it unlocks richer book metadata and bypasses rate limits. See the [Amazon Cookie](/docs/metadata/amazon-cookie) guide. |
...
| **Hardcover** | API Token | Requires an API token from [hardcover.app/account/api](https://hardcover.app/account/api). See the [Hardcover API](/docs/metadata/hardcover-token) guide for setup instructions. |
...
| **Lubimyczytac** | Polish literature and book ratings. See the [LubimyCzytac](/docs/metadata/lubimyczytac) guide. |
| **Ranobedb** | Light novels and web novel metadata. See the [RanobeDB](/docs/metadata/ranobedb) guide. |

:::tip[Provider Availability]
Enabling a provider here makes it available in the priority dropdowns on the [Fetch Configuration](/docs/metadata/metadata-fetch-configuration) page. After enabling a new provider, go to Metadata 2 to assign it a priority for the fields you want it to supply.
:::
...
<DocsImage src={"/img/metadata/enabled-fields"} alt={"Enabled Fields in Metadata Editor & Picker"} />
...
<DocsImage src={"/img/metadata/public-reviews"} alt={"Public Reviews"} />
...
- All configuration changes are recorded in the [Audit Log](/docs/tools/audit-logs).
- Provider settings on this page (Metadata 1) control **which providers are available**. The [Fetch Configuration](/docs/metadata/metadata-fetch-configuration) page (Metadata 2) controls **which providers are used for each field** and in what priority order.
