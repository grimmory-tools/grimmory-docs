...
- [Rauthy](/docs/authentication/rauthy)
...
This can be seen during an OIDC login attempt as a `Filtered Host` exception or when testing connections, which will fail with the error `Filtered Host`.

:::danger[Disabling SSRF Protection]
Disabling SSRF Protection means that any administrative user who can control your OIDC configuration can make a blind GET
request to hosts on private IP address space, such as local computers on your network.
:::

To disable SSRF protections on the OIDC flow, an **environment variable override** must be added: `OIDC_ALLOW_UNSAFE_HOSTS=true`.  Changes to environment variables require a restart to take effect.
