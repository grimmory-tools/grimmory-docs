...
Reading progress sync is bidirectional, and personal ratings set on Kobo sync back to Grimmory. Your shelves and magic shelves also carry over as Kobo collections, so the organization you've built in Grimmory follows you to the device.

You control what gets synced through a dedicated Kobo shelf: add a book to the shelf and it appears on your device, remove it and it disappears on the next sync.

:::note[Kobo Account Required]
...
<DocsImage src={"/img/kobo/device-settings"} alt={"Grimmory Settings > Devices page showing the Kobo Sync Token and Regenerate Token button"} />
...
<DocsImage src={"/img/kobo/config-file-location"} alt={"Finder showing the KOBOeReader drive with the .kobo hidden folder visible"} manual />
...
<DocsImage src={"/img/kobo/config-before"} alt={"The original Kobo eReader.conf showing api_endpoint pointing to storeapi.kobo.com"} manual />
...
<DocsImage src={"/img/kobo/config-after"} alt={"The modified Kobo eReader.conf with api_endpoint pointing to the Grimmory instance"} manual />
...
<DocsImage src={"/img/kobo/assign-shelf"} alt={"Book card context menu showing Assign Shelf option with shelf list"} />
3. Repeat for all books you want on the device. The Kobo shelf shows everything queued for sync.

<DocsImage src={"/img/kobo/shelf-with-books"} alt={"Grimmory's Kobo shelf view showing 7 books ready to sync"} />
4. On your Kobo, go to **My Books**, tap the sync icon in the top-right corner, and tap **Sync now**

<DocsImage src={"/img/kobo/sync-button"} alt={"Kobo device showing the Sync complete dialog with Sync now button"} manual />
5. Your Grimmory books appear on the device, ready to read

<DocsImage src={"/img/kobo/synced-books"} alt={"Kobo device My Books screen showing the synced books from Grimmory"} manual />
---
...
<DocsImage src={"/img/kobo/remove-menu"} alt={"Kobo context menu on a book showing Read now, View Details, Add to collection, Mark as finished, and Remove options"} manual />
2. Select **Remove from My Books** and tap **Remove**

<DocsImage src={"/img/kobo/remove-confirm"} alt={"Kobo removal dialog with Remove download and Remove from My Books options"} manual />
3. Tap **Sync now** to push the change back to Grimmory

<DocsImage src={"/img/kobo/sync-after-removal"} alt={"Kobo after removal showing one fewer book, with Sync complete dialog"} manual />
The book is removed from your Kobo shelf in Grimmory but stays in your library. It's not deleted, just unsynced.

<DocsImage src={"/img/kobo/shelf-after-removal"} alt={"Grimmory Kobo shelf after sync showing the book has been removed from the shelf"} />
### From Grimmory
...
<DocsImage src={"/img/kobo/synced-progress"} alt={"Book detail page showing Kobo Progress at 11% alongside other metadata"} />
...
<DocsImage src={"/img/kobo/user-permissions"} alt={"Settings > Users page showing the user list with permission columns including Kobo Sync"} />
