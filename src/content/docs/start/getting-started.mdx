...
See the [Installation Guide](/installation) for the complete `docker-compose.yml` with all environment variables and configuration options.
...
After submitting, you'll be redirected to the login page. See [Initial Setup](/initial-setup) for details.
...
After saving, the library scans your folders automatically. See [Set Up First Library](/library/setup-first-library) for a full walkthrough, and [Organization Modes](/library/organization-modes) for how to organize your files.
...
**Bookdrop:** Drop files into the bookdrop folder. Grimmory auto-detects them, fetches metadata, and stages them for review. You can edit metadata, assign a destination library, and finalize the import from the UI. See [Bookdrop](/bookdrop).
...
See [Kobo Integration](/integration/kobo) and [OPDS](/integration/opds) for detailed setup guides.
...
| Feature | Description |
|---------|-------------|
| [Metadata Configuration](/metadata/metadata-fetch-configuration) | Set up automatic metadata enrichment from online sources |
| [Bookdrop Advanced Features](/bookdrop-advanced) | Bulk editing, filename pattern extraction, and more |
| [Magic Shelf](/magic-shelf) | Dynamic, auto-updating shelves based on filters |
| [Authentication](/authentication/overview) | OIDC providers, multi-user setup, and permissions |
...
Yes, set up a reverse proxy with HTTPS. See the [Installation Guide](/installation).
