...
See the [Installation Guide](/docs/installation) for the complete `docker-compose.yml` with all environment variables and configuration options.
...
After submitting, you'll be redirected to the login page. See [Initial Setup](/docs/initial-setup) for details.
...
After saving, the library scans your folders automatically. See [Set Up First Library](/docs/library/setup-first-library) for a full walkthrough, and [Organization Modes](/docs/library/organization-modes) for how to organize your files.
...
**Bookdrop:** Drop files into the bookdrop folder. Grimmory auto-detects them, fetches metadata, and stages them for review. You can edit metadata, assign a destination library, and finalize the import from the UI. See [Bookdrop](/docs/bookdrop).
...
See [Kobo Integration](/docs/integration/kobo) and [OPDS](/docs/integration/opds) for detailed setup guides.
...
| Feature | Description |
|---------|-------------|
| [Metadata Configuration](/docs/metadata/metadata-fetch-configuration) | Set up automatic metadata enrichment from online sources |
| [Bookdrop Advanced Features](/docs/bookdrop-advanced) | Bulk editing, filename pattern extraction, and more |
| [Magic Shelf](/docs/magic-shelf) | Dynamic, auto-updating shelves based on filters |
| [Authentication](/docs/authentication/overview) | OIDC providers, multi-user setup, and permissions |
...
Yes, set up a reverse proxy with HTTPS. See the [Installation Guide](/docs/installation).
