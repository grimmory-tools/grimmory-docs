...
:::caution[Security]
If your security needs forbid secrets from being used in environment variables, you may use `SPRING_CONFIG_LOCATION` to read
a local configuration override file.

The local YAML override file may be created with contents such as `spring: { datasource: { password: example-password } }`.

To use this override file Grimmory must have an environment variable `SPRING_CONFIG_LOCATION` added with the value
`classpath:/application.yaml,/my-config-override.yaml` - where `/my-config-override.yaml` is the location of your configuration
...
1. **[Set Up Admin User](initial-setup)** to create your administrator account
2. **[Set Up First Library](library/setup-first-library)** to configure your first library
